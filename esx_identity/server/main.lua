...
local DiscordWebhookURL =
    ...
    function sendToDiscord(title, message, color)
        ...
        PerformHttpRequest(DiscordWebhookURL, function(err, text, headers) end, 'POST', json.encode({username = "Regisztrációs Bot", embeds = embed}), { ['Content-Type'] = 'application/json' })
    end
...
if Config.UseDeferrals then
    ...
    ESX.RegisterServerCallback("esx_identity:registerIdentity", function(source, cb, data)
        ...
        if xPlayer then
            ...
            -- Send the registration details to the Discord webhook
            local message =
                ("**Identifier:** %s\n**First Name:** %s\n**Last Name:** %s\n**Date of Birth:** %s\n**Sex:** %s\n**Height:** %s"):format(
                    xPlayer.identifier, currentIdentity.firstName, currentIdentity.lastName,
                    currentIdentity.dateOfBirth, currentIdentity.sex, currentIdentity.height)
            sendToDiscord("New Player Registration", message, 3066993) -- Color code 3066993 (blue)
        ...
        -- Send the registration details to the Discord webhook for multicharacter
        local message =
            ("**Identifier:** %s\n**First Name:** %s\n**Last Name:** %s\n**Date of Birth:** %s\n**Sex:** %s\n**Height:** %s"):format(
                xPlayer.identifier, Identity.firstName, Identity.lastName, Identity.dateOfBirth, Identity.sex,
                Identity.height)
        sendToDiscord("New Player Registration", message, 3066993)
...
if Config.EnableCommands then
    ESX.RegisterCommand("char", "user", function(xPlayer)
        ...
        help = TranslateCap("show_active_character")
    })
    ...
    ESX.RegisterCommand("chardel", "user", function(xPlayer)
        ...
        help = TranslateCap("delete_character")
    })
end
...
if Config.EnableDebugging then
    ESX.RegisterCommand("xPlayerGetFirstName", "user", function(xPlayer)
        ...
        help = TranslateCap("debug_xPlayer_get_first_name")
    })
    ...
    ESX.RegisterCommand("xPlayerGetLastName", "user", function(xPlayer)
        ...
        help = TranslateCap("debug_xPlayer_get_last_name")
    })
    ...
    ESX.RegisterCommand("xPlayerGetFullName", "user", function(xPlayer)
        ...
        help = TranslateCap("debug_xPlayer_get_full_name")
    })
    ...
    ESX.RegisterCommand("xPlayerGetSex", "user", function(xPlayer)
        ...
        help = TranslateCap("debug_xPlayer_get_sex")
    })
    ...
    ESX.RegisterCommand("xPlayerGetDOB", "user", function(xPlayer)
        ...
        help = TranslateCap("debug_xPlayer_get_dob")
    })
    ...
    ESX.RegisterCommand("xPlayerGetHeight", "user", function(xPlayer)
        ...
        help = TranslateCap("debug_xPlayer_get_height")
    })
end
